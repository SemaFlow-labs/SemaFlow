...
impl PostgresConnection {
    ...
    /// Create a new PostgreSQL connection with configuration.
    pub fn with_config(
        ...
        let mut db_config: deadpool_postgres::Config = if connection_string.starts_with("postgres")
        {
            // URL format
        ...
        db_config.pool = Some(deadpool_postgres::PoolConfig {
    ...
    #[deprecated(note = "Use with_config() instead - pool size must be set at creation time")]
...
impl BackendConnection for PostgresConnection {
    ...
    async fn execute_sql_paginated(
        ...
        tracing::debug!(
            page_size = page_size,
            offset = offset,
            "executing paginated PostgreSQL query"
        );
        ...
        Ok(PaginatedResult {
            ...
            cursor: next_cursor,
            has_more,
            total_rows: None, // PostgreSQL doesn't provide total count efficiently
        })
    }
}
    ...
    match col.type_() {
        ...
        &Type::NUMERIC => {
            ...
            if let Ok(Some(v)) = row.try_get::<_, Option<f64>>(idx) {
                ...
                // This will trigger if NUMERIC couldn't be converted - SQL should cast it
                tracing::warn!(
                    column = col.name(),
                    "NUMERIC column could not be converted - ensure SQL casts to FLOAT8"
                );
                Value::Null
            }
        }
