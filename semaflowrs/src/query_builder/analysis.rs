...
pub fn analyze_fanout_risk(components: &QueryComponents, _flow: &SemanticFlow) -> FanoutAnalysis {
    ...
    let needed_join_aliases =
        expand_join_chains(&direct_aliases, base_alias, &components.join_lookup);
...
fn analyze_single_table_preagg(
    ...
    for mappings in fanout.join_key_mappings.values() {
